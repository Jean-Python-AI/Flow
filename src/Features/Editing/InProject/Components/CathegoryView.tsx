...
export default function CathegoryView({parentId, categoryActivate_info, onCategoryPress}: {parentId: number, categoryActivate_info:number, onCategoryPress: (categoryId: number) => void}) {
  ...
  const reloadCategories = () => {ReadCategory(parentId, modifyCategory); console.log('Categories reloaded...');};
  ...
  useEffect(() => {
    // Activer la première catégorie si aucune n'est activée OU si la catégorie activée a été supprimée
    const activeCategoryStillExists = categorys.some(cat => cat.id === categoryActivate);

    if ((categoryActivate_info === 0 || !activeCategoryStillExists) && categorys.length > 0) {
      const firstCategory = categorys[0];
      onCategoryPress(firstCategory.id);
      setCategoryActivate(firstCategory.id);
    }
  }, [categorys, categoryActivate_info, categoryActivate, onCategoryPress]);
